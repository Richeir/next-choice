...
/**
 * 新浪 K 线密文解码：hk_js_decode 原样取自 akshare 1.18.x
 * （scripts/.venv 内固定版本）的 stock/cons.py::hk_js_decode，
 * 逻辑为一段自包含 JS 函数 d(payload)。上游更换加密版本时需同步更新。
 *
 * Python 侧经 py_mini_racer 执行同一段代码；Node 原生即 JS，
 * 用间接 eval 注入全局后直接调用（见 decodeSinaKlines）。
 */

export const HK_JS_DECODE = String.raw`function d(t) {
...
/** 解码后的原始 bar（未做列标准化）。数值字段由解码器保证为 number。 */
export interface DecodedBar {
  /** YYYY-MM-DD；解码器产出的 Date 为 UTC 零点对齐，取 ISO 日期与时区无关。 */
  date: string;
  open: number;
  high: number;
  low: number;
  ...
  volume: number;
  amount: number;
  /** 解密自带的前收；股票路径会忽略（对齐 akshare 行为），ETF 路径使用。 */
...
function ensureDecoder(): void {
  ...
  // 间接 eval：函数声明落在全局作用域，等价于 mini_racer 的 js_code.eval。
  (0, eval)(HK_JS_DECODE);
  if (typeof (globalThis as unknown as Record<string, unknown>).d !== 'function') {
    throw new CollectionError('sina kline decoder init failed');
  }
  decoderReady = true;
}

/**
 * 从 klc_kl.js 响应文本中提取密文并解码为原始 bar 列表。
 * 空数据抛 {@link EmptyDataError}（可重试），结构损坏抛 {@link CollectionError}。
 */
export function decodeSinaKlines(text: string): DecodedBar[] {
  const match = text.match(/=\s*"([^"]*)"/);
  if (!match || !match[1]) {
    throw new EmptyDataError('klc_kl payload not found');
  }
  ensureDecoder();
  ...
  try {
    rows = (
      globalThis as unknown as {
        d: (payload: string) => Array<Record<string, unknown>>;
      }
    ).d(match[1]);
  } catch (e) {
    throw new CollectionError(
      'sina kline decode failed: ' + (e instanceof Error ? e.message : String(e)),
    );
  }
  if (!Array.isArray(rows) || rows.length === 0) {
    throw new EmptyDataError('klc_kl decoded empty rows');
  }
  return rows.map((r) => ({
    date: new Date(r.date as unknown as string).toISOString().slice(0, 10),
    open: Number(r.open),
    high: Number(r.high),
    low: Number(r.low),
    ...
    prevclose: r.prevclose == null ? null : Number(r.prevclose),
  }));
}
