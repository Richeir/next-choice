...
class TestRunFetch:
    ...
    def test_codes_commits_52w_update(self, conn, tmp_path, monkeypatch):
        """52w 回写后须显式提交：未提交数据对第二个连接不可见。"""
        from datetime import date, timedelta
        today = date.today()
        in_win = (today - timedelta(days=100)).isoformat()
        cover = (today - timedelta(days=400)).isoformat()
        ...
        fetch_data.run_fetch(conn, ["600000"], ["daily"], ["3"],
                             cover, today.isoformat())
        c2 = sqlite3.connect(str(tmp_path / "test.db"))
        row = c2.execute("SELECT high_52w, low_52w FROM stock_info"
                         " WHERE code='600000'").fetchone()
        c2.close()
        assert row == (20, 5)
