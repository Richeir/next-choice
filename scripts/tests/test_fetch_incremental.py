...
class TestDueFreqs:
    ...
    def test_weekly_due_on_saturday(self):
        today = date(2026, 8, 22)  # 周六，数据新鲜也更新
        assert fetch_data._due_freqs(["weekly"], today, {"weekly": "2026-08-21"}) == ["weekly"]

    def test_weekly_due_on_sunday(self):
        today = date(2026, 8, 23)  # 周日
        assert fetch_data._due_freqs(["weekly"], today, {"weekly": "2026-08-21"}) == ["weekly"]
    ...
    def test_mixed_freqs_partial_due(self):
        today = date(2026, 8, 22)  # 周六：daily 门控跳过，weekly 更新
